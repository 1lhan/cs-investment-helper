                    ...
                    {events.slice().sort((a, b) => new Date(b.dates.release) - new Date(a.dates.release)).map((event, index) =>
                        event.eventType == 'tournament' ?
                            <div className="item-div" key={index}>
                                ...
                                <span className="item-div-children time-difference">
                                    {event.dates['tournament-start'] && (new Date(event.dates['tournament-end']) - new Date(event.dates['tournament-start'])) / (1000 * 60 * 60 * 24)}
                                </span>
                                <span className="item-div-children">{event.dates['tournament-end'] && event.dates['tournament-end']}</span>
                                <span className="item-div-children time-difference">
                                    {event.dates['tournament-end'] == null ? '' : event.dates['sale-start'] ? (new Date(event.dates['sale-start']) - new Date(event.dates['tournament-end'])) / (1000 * 60 * 60 * 24) : ''}
                                </span>
                                <span className="item-div-children">{event.dates['sale-start']}</span>
                                <span className="item-div-children time-difference" style={{ color: green }}>
                                    {event.dates['sale-end'] ? (new Date(event.dates['sale-end']) - new Date(event.dates['sale-start'])) / (1000 * 60 * 60 * 24) : ''}
                                </span>
                                <span className="item-div-children">{event.dates['sale-end']}</span>
                                <span className="item-div-children time-difference" style={{ color: green }}>
                                    {event.dates['sale-end'] ? (new Date(event.dates['sale-end']) - new Date(event.dates['release'])) / (1000 * 60 * 60 * 24) :
                                        Math.floor((new Date() - new Date(event.dates['release'])) / (1000 * 60 * 60 * 24))}
                                </span>
                            </div>
                            :
                            <div className="item-div" key={index}>
